...
if (!$canAprove) {
    echo json_encode(['status' => 'error', 'message' => 'Sem permissão para desaprovar']); exit;
}
...
$stmt = $db->prepare("UPDATE conciliacao_bancaria SET aprovado = 0, aprovador = NULL WHERE id = ?");
try {
    $stmt->execute([$id]);
