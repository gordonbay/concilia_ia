...
try {
    $stmt = $db->prepare("SELECT usuario FROM conciliacao_bancaria WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$row) { echo json_encode(['status' => 'error', 'message' => 'Registro não encontrado']); exit; }

    if (!$isAdmin && $row['usuario'] != $userId) {
        echo json_encode(['status' => 'error', 'message' => 'Sem permissão para excluir este registro']); exit;
    }

    $stmt = $db->prepare("DELETE FROM conciliacao_bancaria WHERE id = ?");
    $stmt->execute([$id]);
