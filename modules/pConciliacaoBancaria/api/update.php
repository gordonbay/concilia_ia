...
$canEdit = in_array('conciliacao_editor',    $perms)
        || in_array('conciliacao_finalizar', $perms)
        || in_array('conciliacao_aprovar',   $perms)
        || $isAdmin;
if (!$canEdit) {
    echo json_encode(['status' => 'error', 'message' => 'Sem permissão para editar conciliações.']);
    exit;
}
$db = new PDO(
...
$adiantamento        = isset($input['adiantamento']) ? (int)(bool)$input['adiantamento'] : 0;
// NOVO: Recebendo tipo_lancamento e dados_fiscais_manual
$tipo_lancamento      = $input['tipo_lancamento']      ?? null;
$dados_fiscais_manual = $input['dados_fiscais_manual'] ?? null;

// ─── Normaliza campo empresa ──────────────────────────────────────────────────
$empresa_raw = $input['empresa'] ?? null;
if (!is_null($empresa_raw)) {
    $empresa_raw     = trim((string)$empresa_raw);
    $empresa_decoded = json_decode($empresa_raw, true);
    if (is_array($empresa_decoded) && isset($empresa_decoded['cnpj'])) {
        $empresa = json_encode([
            'cnpj'     => $empresa_decoded['cnpj']              ?? '',
            'banco_id' => (string)($empresa_decoded['banco_id'] ?? ''),
            'conta'    => $empresa_decoded['conta']             ?? '',
...
// ─── Normaliza recebedor quando é empresa nossa ───────────────────────────────
$recebedor_empresa_json = $input['recebedor_empresa_json'] ?? null;
if (!empty($recebedor_empresa_json)) {
    $recDec = json_decode($recebedor_empresa_json, true);
    if (is_array($recDec) && isset($recDec['cnpj'])) {
        $recebedor_doc = json_encode([
            'cnpj'     => $recDec['cnpj']              ?? '',
            'banco_id' => (string)($recDec['banco_id'] ?? ''),
            'conta'    => $recDec['conta']             ?? '',
        ], JSON_UNESCAPED_UNICODE);
    }
}
...
try { $db->exec("ALTER TABLE conciliacao_bancaria ADD COLUMN recebedor_doc VARCHAR(500) DEFAULT NULL"); } catch (Exception $e) {}
...
try { $db->exec("ALTER TABLE conciliacao_bancaria ADD COLUMN adiantamento TINYINT(1) NOT NULL DEFAULT 0"); } catch (Exception $e) {}
// NOVO: Cria colunas para tipo e dados fiscais manuais
...
// ─── UPDATE ou INSERT dependendo da presença do id ───────────────────────────
if ($id) {
    $sql = "
        ...
        SET
            ...
            adiantamento        = ?,
            tipo_lancamento     = ?,
            dados_fiscais_manual= ?
        WHERE id = ?
    ";
    $stmt = $db->prepare($sql);
    try {
        $stmt->execute([
            $empresa, $operacao, $valor_total, $data_vencimento,
            $parcelas, $fatura, $natureza_financeira, $observacao,
            $operacao_extra, $anexos, $anotacao, $recebedor_doc, $recebedor_nome,
    ...
    // id ausente = cria novo registro (fallback)
    $data_criacao = date('Y-m-d H:i:s');
    $sql = "
        INSERT INTO conciliacao_bancaria
            (empresa, usuario, data_criacao, operacao, valor_total, data_vencimento,
             parcelas, fatura, natureza_financeira, observacao, operacao_extra,
             anexos, anotacao, recebedor_doc, recebedor_nome, adiantamento,
